...
If the `trigger` is `on_demand`, the run needs a request. Where a golden example's Captured line says what it was a reply to, send a request of that kind, on the installer's own material, and say which request you sent. Where none says, ask the installer for something they would normally ask it.
...
|---|---|---|
| Read your notes folder | lost | nothing here reaches your files, and pasting them in each run does not survive an unattended one, so nothing reads your notes and the section they fed is empty |
